 ...
 */
auto AppWindow::restore_maximized_state_if_needed() -> void
{
    bool should_queue_restore = false;

#ifdef Q_OS_WIN
    const Qt::WindowStates current_state = windowState();
...
auto AppWindow::event(QEvent* event) -> bool
{
    ...
    if (is_window_state_change)
    {
        const Qt::WindowStates current_window_state = windowState();
...
auto AppWindow::nativeEvent(const QByteArray& eventType, void* message, qintptr* result) -> bool
{
    ...
    {
        const bool platform_ready = (handle() != nullptr);
        ...
        if (msg->message == WM_SIZE)
        {
            if (msg->wParam == SIZE_MINIMIZED)
            {
                m_was_maximized_before_minimize =
                    m_was_maximized_before_minimize || m_is_maximized_native;
                m_restore_maximized_queued = false;
            }
 ...
 */
auto AppWindow::native_window_handle() const -> HWND
{
    HWND result = reinterpret_cast<HWND>(winId());
    return result;
}
 ...
 */
auto AppWindow::system_resize_border_width() const -> int
{
    HWND hwnd = native_window_handle();
 ...
 */
auto AppWindow::system_resize_border_height() const -> int
{
    HWND hwnd = native_window_handle();
    const UINT dpi = get_window_dpi(hwnd);
 ...
 */
auto AppWindow::system_caption_height() const -> int
{
    HWND hwnd = native_window_handle();
    const UINT dpi = get_window_dpi(hwnd);
...
auto AppWindow::enable_win11_features() -> void
{
    ...
    if (IsWindow(hwnd))
    {
        ...
        {
            DWORD corner_pref = m_use_rounded_corners
 ...
 */
auto AppWindow::is_over_qml_item(const QQuickItem* item, int global_x, int global_y) const -> bool
{
    bool is_over = false;

    if (item != nullptr)
    {
        const QPointF scene_pos = mapFromGlobal(QPointF(global_x, global_y));
 ...
 */
auto AppWindow::is_over_qml_item_or_child(const QQuickItem* item, int global_x,
...
auto AppWindow::handle_nc_hit_test(MSG* msg) -> LRESULT
{
    ...
    const int caption = (m_title_bar_item != nullptr) ? static_cast<int>(m_title_bar_item->height())
    ...
    const bool over_button = is_over_qml_item(m_minimize_button_item, x, y) ||
                             is_over_qml_item(m_maximize_button_item, x, y) ||
                             is_over_qml_item(m_close_button_item, x, y);

    const bool over_custom_item = is_over_qml_item_or_child(m_custom_title_bar_item, x, y);
    ...
    if (m_title_bar_item != nullptr && !over_interactive_item)
    {
        if (is_over_qml_item(m_title_bar_item, x, y))
        {
    ...
    else if (m_title_bar_item == nullptr && !over_interactive_item)
    {
 ...
 */
auto AppWindow::handle_get_min_max_info(MSG* msg) -> void
{
    ...
    if (GetMonitorInfoW(mon, &mi))
    {
        ...
        const int min_width = static_cast<int>(minimumWidth());
        const int min_height = static_cast<int>(minimumHeight());
